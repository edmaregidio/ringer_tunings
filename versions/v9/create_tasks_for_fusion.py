...
path_to_rg = '~jodafons/tasks/user.jodafons.data17_13TeV.AllPeriods.sgn.probes_lhmedium_EGAM1.bkg.VProbes_EGAM7.GRL_v97.v9.rg_et{ET}_eta{ETA}.r1'
path_to_ss = '~jodafons/tasks/user.jodafons.data17_13TeV.AllPeriods.sgn.probes_lhmedium_EGAM1.bkg.VProbes_EGAM7.GRL_v97.v9.ss_et{ET}_eta{ETA}.r1'
...
command = """maestro.py task create \
  ...
  --extraArgs '--type fusion --path_to_rg {RINGS} --path_to_ss {SHOWER}'" \
  --queue "gpu" """
...
for et in range(5):
    for eta in range(5):
        ...
        cmd = command.format(ET=et,ETA=eta,REF=ref, RINGS=path_to_rg.format(ET=et,ETA=eta), SHOWER=path_to_ss.format(ET=et,ETA=eta) )
        print(cmd)
        #os.system(cmd)
